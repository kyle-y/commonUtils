 ...
 */
package com.example.administrator.common.commonAdapter;
...
public class BaseAdapterHelper {
	...
	public BaseAdapterHelper setSelected(int viewId, boolean value) {
		TextView view = retrieveView(viewId);
		view.setSelected(value);
		return this;
	}

	public BaseAdapterHelper setText(int viewId, SpannableString value) {
		TextView view = retrieveView(viewId);
		view.setText(value);
		return this;
	}

	public BaseAdapterHelper setTextIf(int viewId, String value, boolean isAppen, String content) {
		TextView view = retrieveView(viewId);
		if (isAppen) {
			value = content + value;
		}
	...
	public BaseAdapterHelper setEdittextText(int viewId, String value) {
	...
	public BaseAdapterHelper setBackgroundColorAndIf(int viewId, int color, boolean isChange, int afterColor) {
		View view = retrieveView(viewId);
		if (isChange) {
	...
	public BaseAdapterHelper setBackgroundResIf(int viewId, int backgroundRes, boolean isSelect, int backgroundResSelect) {
		View view = retrieveView(viewId);
		if (isSelect) {
			view.setBackgroundResource(backgroundResSelect);
		} else {
			view.setBackgroundResource(backgroundRes);
		}
		return this;
	 ...
	 * @return The BaseAdapterHelper for chaining.
	 */
	public BaseAdapterHelper setTextColorIf(int viewId, int textColor, boolean isChange, int afterTextColor) {
		TextView view = retrieveView(viewId);
		if (isChange) {
			view.setTextColor(context.getResources().getColor(afterTextColor));
		} else {
	...
	public BaseAdapterHelper setTextDrawable(int viewId, int imageRes, boolean isChange, int imageResAfter) {
		TextView view = retrieveView(viewId);
		int myRes = imageRes;
		if (isChange) {
			myRes = imageResAfter;
		}
		Drawable drawable = context.getResources().getDrawable(myRes);
	 ...
	 * @return The BaseAdapterHelper for chaining.
	 */
	public BaseAdapterHelper setImageUrlPicasso(int viewId, String imageUrl) {
		ImageView view = retrieveView(viewId);
//		Picasso.with(context).load(imageUrl).into(view);
		return this;
	}

	public BaseAdapterHelper setImageUrlPhotoPicasso(int viewId, String imageUrl) {
		ImageView view = retrieveView(viewId);
		if (imageUrl != null && !imageUrl.equals("")) {
//			Picasso.with(context).load(imageUrl).error(R.mipmap.icon_photo).placeholder(R.mipmap.icon_photo)
//					.resize(CommonUtil.dipsToPixels(context, 50), CommonUtil.dipsToPixels(context,50))
//					.centerCrop().into(view);
		} else {
//			view.setImageResource(R.mipmap.icon_photo);
		}
	...
	/**
	 * Add an action to set the image of an image view. Can be called multiple times.k
	 */
	...
	public BaseAdapterHelper setEnable(int viewId, boolean isEnable) {
		View view = retrieveView(viewId);
	...
	public BaseAdapterHelper addTextChangedListener(int viewId, TextWatcher watcher) {
		EditText editText = retrieveView(viewId);
		editText.addTextChangedListener(watcher);
		return this;
	}

	public BaseAdapterHelper setOnFocusChangeListener(int viewId, View.OnFocusChangeListener listener) {
		EditText editText = retrieveView(viewId);
